...
The Linker EG manual describes 15 captured values: three channels per finger. On the tested right EG glove, the raw serial order is `thumb, pinky, middle, ring, index`, not simple anatomical order. The bridge fixes that raw order first, then maps logical finger names directly: thumb drives thumb, index drives index, middle drives middle, ring drives ring, and pinky drives pinky. The L10 has 10 active DOF, so `_1` channels drive non-thumb finger pitch, `_0` channels drive side swing, and extra EG channels that the L10 cannot reproduce are ignored.
...
`control_l10_left_from_eg_glove.py` also applies that reference map at runtime by default. This prevents an older local `config/l10_left_eg_glove_mapping.auto.yaml` from keeping the wrong finger order after `git pull`. Set `apply_reference_mapping: false` in the YAML only when deliberately testing custom `glove_key` values.
...
Motor `2` is the L10 Index Finger MCP Pitch. Motor `6` is the L10 Index Finger Adduction/Abduction. With the corrected raw order, those are driven by `index_1` and `index_0`; if only left-index bend is wrong, use `--motors 2`.
...
glove 10 -> L10 joint 4 Ring Finger MCP Pitch
